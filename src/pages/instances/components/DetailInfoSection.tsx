...
function MonoText({ children }: { children: ReactNode }) {
  return (
    <Typography sx={{ fontFamily: MONO, fontSize: 12 }}>{children}</Typography>
  );
}
...
export default function DetailInfoSection({ instance }: Props) {
  ...
  const infoItems: Array<{
    ...
    },
    {
      ...
      value: (
        <Box display="flex" alignItems="center" gap={1}>
          ...
          <Typography
            sx={{ fontFamily: MONO, fontSize: 11, color: "text.secondary" }}
          >
    ...
    },
    {
      ...
      value: (
        <StatusChip
          status={
            // instance.control_signal === "pause"
            //   ? "pausing"
            //   : instance.control_signal === "terminate"
            //     ? "terminating" : 
            instance.status
          }
        />
    ...
    },
    {
      ...
      value: (
        <Typography fontSize={13}>
          {formatSafeDate(instance.startedAt)}
        </Typography>
      ),
    },
  ];
  ...
  if (instance.endedAt) {
    infoItems.push({
      ...
      value: (
        <Typography fontSize={13}>
          {formatSafeDate(instance.endedAt)}
        </Typography>
      ),
    });
  }
        ...
        <Box>
          <Box display="flex" alignItems="center" gap={1}>
            <Typography fontSize={13} fontWeight={500}>
              {instance.currentTask.name ||
                `${instance.currentTask.type || "Task"} Node`}
            </Typography>
          ...
          >
            {instance.currentTask.type || "unknown"} •{" "}
            {instance.currentTask.nodeId || "-"}
          </Typography>
        </Box>
      ),
    });
      ...
      >
        ...
        >
          <JsonAccordion
            ...
            data={instance.inputVariables}
          />
          <JsonAccordion
            title="Current Variables"
            data={instance.currentVariables}
          />
          <JsonAccordion
            title="Output Variables"
            data={instance.outputVariables}
          />
        </AccordionDetails>
      </Accordion>
    </Paper>

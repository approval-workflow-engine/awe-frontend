    ...
    <Accordion disableGutters elevation={0}
      ...
      <AccordionDetails sx={{ pt: 0 }}>
        <Box component="pre" sx={{
          fontFamily: MONO, fontSize: 12, m: 0, p: 1.5,
  ...
  return (
    <Box sx={{
      ...
      <Box display="flex" alignItems="center" gap={2} mb={0.5}>
        ...
        <Typography fontSize={12} color="text.secondary" sx={{ fontFamily: MONO }}>
          Started: {safeDate(exec.startedOn)}
          {exec.endedOn ? ` · Ended: ${safeDate(exec.endedOn)}` : ''}
        </Typography>
      </Box>
      {exec.message && (
        <Typography fontSize={13} sx={{ mt: 0.5, fontStyle: 'italic', color: 'text.secondary' }}>
          {exec.message}
        </Typography>
      )}
      {exec.error && (
        <Typography fontSize={12} sx={{ mt: 0.5, color: 'error.main', fontWeight: 600 }}>
          Error: {exec.error}
        </Typography>
      )}
    ...
    <>
      ...
      >
        ...
        <TableCell>
          ...
          {task.message && (
            <Typography fontSize={12} sx={{ mt: 0.5, fontStyle: 'italic', color: 'text.secondary' }}>
              {task.message}
            </Typography>
          )}
          {task.error && (
            <Typography fontSize={11} sx={{ mt: 0.5, color: 'error.main', fontWeight: 600 }}>
              Error: {task.error}
            </Typography>
          )}
        </TableCell>
          ...
          <Collapse in={open} timeout="auto" unmountOnExit>
            <Box sx={{ bgcolor: 'action.selected' }}>
              {task.taskExecutionLog.length === 0 ? (
                <Typography fontSize={12} color="text.disabled" sx={{ px: 3, py: 2 }}>
...
export default function InstanceAuditPage() {
  ...
  const filteredTasks = audit?.taskLog.filter((t) => {
      ...
      <PageHeader
        ...
        action={
          <Tooltip title="Reload">
            <IconButton size="small" onClick={fetchAudit} disabled={loading} sx={{ color: 'text.secondary' }}>
        ...
        <Box display="flex" flexDirection="column" gap={2}>
          <Paper variant="outlined" sx={{ p: 3 }}>
            <Box display="flex" alignItems="center" gap={1.5} mb={2}>
              <Typography fontWeight={700} fontSize={15}>Instance Details</Typography>
            ...
            <Box sx={{ '& > *:not(:last-child)': { borderBottom: '1px solid', borderColor: 'divider' } }}>
              <InfoRow label="Instance ID"
                value={<Typography sx={{ fontFamily: MONO, fontSize: 12 }}>{inst.id}</Typography>} />
              ...
              />
              <InfoRow label="Auto Advance"
                value={<Typography sx={{ fontFamily: MONO, fontSize: 12 }}>{inst.autoAdvance ? 'Yes' : 'No'}</Typography>} />
              
              {/* Instance Metadata Section */}
              <InfoRow label="Total Tasks"
                value={
                  <Box display="flex" gap={1}>
                    <Chip label={`${inst.totalTasks} total`} size="small" variant="outlined" sx={{ height: 22, fontSize: 11 }} />
                    {inst.totalTasks > 0 && (
                      <Box display="flex" gap={0.5}>
                        {inst.taskStatusBreakdown.completed > 0 && (
                          <Chip label={`${inst.taskStatusBreakdown.completed} completed`} size="small" sx={{ height: 22, fontSize: 10 }} color="success" />
                        )}
                        {inst.taskStatusBreakdown.failed > 0 && (
                          <Chip label={`${inst.taskStatusBreakdown.failed} failed`} size="small" sx={{ height: 22, fontSize: 10 }} color="error" />
                        )}
                        {inst.taskStatusBreakdown.in_progress > 0 && (
                          <Chip label={`${inst.taskStatusBreakdown.in_progress} in progress`} size="small" sx={{ height: 22, fontSize: 10 }} color="warning" />
                        )}
                        {inst.taskStatusBreakdown.terminated > 0 && (
                          <Chip label={`${inst.taskStatusBreakdown.terminated} terminated`} size="small" sx={{ height: 22, fontSize: 10 }} />
                        )}
                      </Box>
                    )}
                  </Box>
              ...
              />
              <InfoRow label="Total Executions"
                value={<Typography sx={{ fontFamily: MONO, fontSize: 12 }}>{inst.totalExecutions}</Typography>} />
              {inst.durationMs !== null && (
                <InfoRow label="Duration"
                  value={<Typography sx={{ fontFamily: MONO, fontSize: 12 }}>{formatDuration(inst.durationMs)}</Typography>} />
              )}
              
              <InfoRow label="Started"
                value={<Typography fontSize={13}>{safeDate(inst.startedAt)}</Typography>} />
              {inst.completedAt && (
                <InfoRow label="Completed"
                  value={<Typography fontSize={13}>{safeDate(inst.completedAt)}</Typography>} />
              )}
              {inst.failedAt && (
                <InfoRow label="Failed"
                  value={<Typography fontSize={13}>{safeDate(inst.failedAt)}</Typography>} />
              )}
              {inst.terminatedAt && (
                <InfoRow label="Terminated"
                  value={<Typography fontSize={13}>{safeDate(inst.terminatedAt)}</Typography>} />
              )}
            </Box>

            <JsonAccordion title="Input Variables" data={inst.inputVariables} />
            <JsonAccordion title="Output Variables" data={inst.outputVariables} />
          </Paper>

          {/* Task log table */}
          <Paper variant="outlined" sx={{ overflow: 'hidden' }}>
            <Box sx={{ px: 2, py: 1.5, borderBottom: '1px solid', borderColor: 'divider', display: 'flex', alignItems: 'center', gap: 1 }}>
              <Typography fontWeight={700} fontSize={15}>Task Log</Typography>
              <Chip
                    ...
                    <TableCell colSpan={6}>
                      <Box sx={{ py: 6, textAlign: 'center' }}>
                        <Typography fontSize={13} color="text.secondary">
                          {search ? 'No tasks match your search.' : 'No tasks recorded.'}
                        </Typography>
                      </Box>
                    </TableCell>
                  </TableRow>

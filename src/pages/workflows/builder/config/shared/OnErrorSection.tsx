...
export default function OnErrorSection({ value, onChange, availableContext }: Props) {
  ...
  const remove = (idx: number) => updateMap(errorMap.filter((_, i) => i !== idx));
  ...
  return (
    <Box display="flex" flexDirection="column" gap={1}>
      <Box display="flex" gap={0.5}>
        <Button size="small" variant={mode === 'terminate' ? 'contained' : 'outlined'}
          onClick={() => onChange('terminate')}
          sx={{
            fontSize: 10, height: 26, borderRadius: '6px', flex: 1, fontWeight: 600, textTransform: 'none',
            ...(mode === 'terminate' ? activeStyle('239,68,68') : { borderColor: 'divider', color: 'text.secondary' }),
          }}>
          Terminate
        </Button>
        <Button size="small" variant={mode === 'map' ? 'contained' : 'outlined'}
          onClick={() => onChange({ errorMap: [] })}
          sx={{
            fontSize: 10, height: 26, borderRadius: '6px', flex: 1, fontWeight: 600, textTransform: 'none',
            ...(mode === 'map' ? activeStyle('79,110,247') : { borderColor: 'divider', color: 'text.secondary' }),
          }}>
          Map to Context
        </Button>
      </Box>
      ...
      {mode === 'terminate' && (
        <Typography sx={{ fontSize: 10, color: 'text.disabled', fontStyle: 'italic', px: 0.5 }}>
        ...
        <Box display="flex" flexDirection="column" gap={0.75}>
          {errorMap.map((row, idx) => (
            <Box key={idx} sx={{
              ...
              p: 0.75, display: 'flex', flexDirection: 'column', gap: 0.75,
              backgroundColor: 'action.hover',
            }}>
              <Box display="flex" gap={0.5}>
                <Button size="small" variant={row.fromType === 'jsonPath' ? 'contained' : 'outlined'}
                  onClick={() => update(idx, { fromType: 'jsonPath' })}
                  sx={{
                    fontSize: 9, height: 22, borderRadius: '5px', flex: 1, textTransform: 'none',
                ...
                <Button size="small" variant={row.fromType === 'expression' ? 'contained' : 'outlined'}
              ...
              {row.fromType === 'jsonPath' && (
                <Box display="flex" gap={0.5}>
                  <Box flex={1}>
                    <JsonPathInput value={row.jsonPath ?? ''} onChange={v => update(idx, { jsonPath: v })} placeholder="error.message" />
                  </Box>
                  <DataTypeSelect value={row.dataType ?? DataType.STRING} onChange={v => update(idx, { dataType: v })} />
                </Box>
              )}
              {row.fromType === 'expression' && (
                <ExpressionInput
          ...
          <AddRowButton
            ...
            onClick={() => updateMap([...errorMap, { fromType: 'jsonPath', jsonPath: '', contextVariable: EMPTY_CV }])}
          />
        </Box>
      )}
    </Box>

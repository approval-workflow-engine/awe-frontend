...
import {
  Box, Typography, IconButton, TextField, Button, Divider,
} from '@mui/material';
...
export default function ServiceTaskConfig({ node, availableContext, onUpdateConfig }: Props) {
  const c = node.config;
  ...
  const updateHeader = (idx: number, patch: Partial<HeaderRow>) =>
    set('headers', headers.map((h, i) => i === idx ? { ...h, ...patch } : h));
  const removeHeader = (idx: number) =>
    set('headers', headers.filter((_, i) => i !== idx));
    ...
    <Box display="flex" flexDirection="column" gap={1.5}>
        ...
        <Box display="flex" gap={0.5}>
          {HTTP_METHODS.map(m => (
            <Button
              ...
              sx={{
                fontSize: 9, height: 22, borderRadius: '5px', minWidth: 0, px: 0.75,
                fontWeight: 700, textTransform: 'none',
                backgroundColor: method === m ? `${METHOD_COLORS[m]}22` : 'transparent',
      ...
      <CollapsibleSection title="Headers" count={headers.length}>
        <Box display="flex" flexDirection="column" gap={0.75}>
          {headers.map((h, idx) => (
            <Box key={idx} sx={{
              ...
              <Box display="flex" gap={0.5} alignItems="center" mb={0.5}>
                <TextField
                  size="small" placeholder="Header name" value={h.key}
                ...
                />
                <IconButton
                  className="delete-btn" size="small" onClick={() => removeHeader(idx)}
          ...
          <AddRowButton
            label="Add Header"
            onClick={() => set('headers', [...headers, { key: '', valueExpression: '' }])}
          />
        </Box>
      </CollapsibleSection>
      ...
      {method !== 'GET' && method !== 'DELETE' && (
        <CollapsibleSection title="Request Body" count={body.length}>
          <Box display="flex" flexDirection="column" gap={0.5}>
            ...
            {bodyError ? (
              <Typography sx={{ fontSize: 9, color: '#ef4444' }}>{bodyError}</Typography>
            ) : body.length > 0 ? (
      ...
      <CollapsibleSection title="Retry & Timeout">
        <Box display="flex" flexDirection="column" gap={0.75}>
          ...
          <Box display="flex" alignItems="center" justifyContent="space-between">
            <Typography sx={{ fontSize: 11, color: 'text.secondary' }}>Timeout (ms)</Typography>
            <Box sx={{ width: 100 }}>
              <NumberInput
          ...
          <Box display="flex" alignItems="center" justifyContent="space-between">
            <Typography sx={{ fontSize: 11, color: 'text.secondary' }}>Retry Delay (ms)</Typography>
            <Box sx={{ width: 100 }}>
              <NumberInput

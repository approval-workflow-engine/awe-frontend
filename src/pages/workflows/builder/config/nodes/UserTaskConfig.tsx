...
export default function UserTaskConfig({
  ...
  const toggleRes = (fieldId: string) =>
    setExpandedRes((s) => {
      const n = new Set(s);
  ...
  const handleResTypeChange = (idx: number, newType: DataType) => {
    ...
    updateRes(idx, {
      type: newType,
      ...(!isManual && { uiType: getDefaultUiType(newType) }),
    });
  };
              ...
              <Box sx={{ px: 0.75, py: 0.75, backgroundColor: "action.hover" }}>
                <Box display="flex" gap={0.5} alignItems="center" mb={0.5}>
                  ...
                  <DataTypeSelect
                    ...
                    onChange={(v) => handleResTypeChange(idx, v)}
                    exclude={[DataType.NULL]}
                  />
                  <Tooltip
                    title={
                      expandedRes.has(row.fieldId) ? "Collapse" : "More options"
                    }
                  >
                    <IconButton
                      size="small"
                      onClick={() => toggleRes(row.fieldId)}
                      sx={{
                        p: 0.25,
                        color: "text.disabled",
                    ...
                    >
                      <KeyboardArrowRightIcon
                        sx={{
                          fontSize: 13,
                          transform: expandedRes.has(row.fieldId)
                            ? "rotate(90deg)"
                            : "none",
                ...
                >
                  ...
                  >
                    <Box display="flex" gap={0.5} alignItems="center">
                      ...
                      <FormControl size="small" sx={{ minWidth: 90 }}>
                        <Select
                          value={row.uiType ?? getDefaultUiType(row.type)}
                          onChange={(e) =>
                            handleResUiTypeChange(idx, e.target.value)
                          }
                    ...
                    <Box display="flex" alignItems="center" gap={0.5}>
                      ...
                      <Switch
                        ...
                        onChange={(e) =>
                          updateRes(idx, { required: e.target.checked })
                        }
                      />
                    </Box>
                  </Box>
                  {!row.required && (
                    <TextField
                      ...
                      onChange={(e) =>
                        updateRes(idx, { default: e.target.value })
                      }
                      sx={{
                        "& .MuiOutlinedInput-root": {
                          borderRadius: "6px",
                  ...
                  <ExpressionInput
                    ...
                    onChange={(v) =>
                      updateRes(idx, { validationExpression: v })
                    }
                    placeholder="value !== ''"
                    availableContext={availableContext}

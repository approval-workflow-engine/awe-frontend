...
import type { CanvasNode, WorkflowInput, ContextVariable } from '../../builderTypes';
...
export default function StartConfig({ node, onUpdateConfig, onChangeInputs, availableContext }: Props) {
  const rows: InputDataMapRow[] = (node.config.inputDataMap as InputDataMapRow[]) ?? [];
  ...
  return (
    <Box display="flex" flexDirection="column" gap={1.25}>
      ...
      {rows.map((row, idx) => (
        <Box key={idx} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '8px', overflow: 'hidden' }}>
          <Box sx={{ px: 0.75, py: 0.75, backgroundColor: 'action.hover', display: 'flex', flexDirection: 'column', gap: 0.75 }}>
            <Box display="flex" gap={0.5} alignItems="center">
              <TextField size="small" placeholder="Label (optional)" value={row.label ?? ''}
              ...
              <Tooltip title={expanded.has(idx) ? 'Collapse' : 'Validation & default'}>
            ...
            <Box>
              ...
              <ContextVariableSelector
                ...
                onChange={cv => update(idx, { contextVariable: cv, jsonPath: cv.name })}
              />
              {row.contextVariable?.name && (
                <Typography sx={{ fontSize: 9, color: 'text.disabled', fontFamily: EXPR_FONT, mt: 0.4, pl: 0.25 }}>
                  {'jsonPath: '}
                  <Box component="span" sx={{ color: 'primary.main' }}>{row.contextVariable.name}</Box>
                </Typography>
              )}
            </Box>
            <Box display="flex" alignItems="center" justifyContent="space-between">
              <Typography sx={{ fontSize: 11, color: 'text.secondary' }}>Required</Typography>
              <Switch size="small" checked={row.required ?? false} onChange={e => update(idx, { required: e.target.checked })} />
            </Box>
          </Box>
          {expanded.has(idx) && (
            <Box sx={{ px: 0.75, py: 0.75, display: 'flex', flexDirection: 'column', gap: 0.75, borderTop: '1px solid', borderColor: 'divider' }}>
              {!row.required && (
                <TextField size="small" label="Default value" value={(row.default as string) ?? ''}
      ...
      <AddRowButton
        label="Add Input"
        onClick={() => sync([...rows, { jsonPath: '', type: DataType.STRING, contextVariable: EMPTY_CV, required: false }])}
      />
    </Box>
  );
}

...
import type { CanvasNode, CanvasEdge } from '../../builderTypes';
import { generateId } from '../../builderTypes';
...
export default function GatewayConfig({
  node, availableContext, edges, onUpdateConfig, onUpdateEdge, onDeleteEdge,
}: Props) {
  const c = node.config;
  const rules: GatewayRule[] = (c.rules as GatewayRule[]) ?? [];
  const defaultRule: DefaultRule = (c.defaultRule as DefaultRule) ?? { id: 'default' };
  ...
  const removeRule = (idx: number) => {
    ...
    set('rules', rules.filter((_, i) => i !== idx));
  ...
  return (
    <Box display="flex" flexDirection="column" gap={1.5}>
          ...
          {rules.map((rule, idx) => (
            <Box key={rule.id} sx={{
              ...
              <Box display="flex" gap={0.5} alignItems="center" mb={0.5}>
                <TextField
                  size="small" placeholder="Branch label (optional)" value={rule.label ?? ''}
                ...
                />
                <IconButton
                  className="delete-btn" size="small" onClick={() => removeRule(idx)}
      ...
      {/* Default branch */}
      <Box sx={{
        border: '1px dashed', borderColor: 'divider', borderRadius: '6px', p: 0.75,
      }}>
        ...
        <TextField
          size="small" placeholder="Label (optional)" value={defaultRule.label ?? ''}
          onChange={e => set('defaultRule', { ...defaultRule, label: e.target.value })}
          sx={{ width: '100%', '& .MuiOutlinedInput-root': { borderRadius: '6px', fontSize: 11, '& fieldset': { borderColor: 'divider' } } }}
          inputProps={{ style: { padding: '4px 8px', fontSize: 11 } }}
        />

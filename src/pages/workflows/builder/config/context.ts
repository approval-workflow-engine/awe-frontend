...
import type { CanvasNode, CanvasEdge, WorkflowInput, ContextVariable } from '../builderTypes';
...
function getAncestorIds(nodeId: string, edges: CanvasEdge[]): { all: Set<string>; direct: Set<string> } {
...
export function getAvailableContext(
  ...
  const { all: ancestorIds, direct: directParentIds } = getAncestorIds(nodeId, edges);
  ...
  if (startNode) {
    const idm = (startNode.config.inputDataMap as Array<{ contextVariable?: ContextVariable; type?: string }>) ?? [];
    idm.forEach(m => {
      if (m.contextVariable?.name && !vars.some(v => v.name === m.contextVariable!.name)) {
        vars.push({
  ...
  nodes.filter(n => ancestorIds.has(n.id)).forEach(n => {
    const isDirect = directParentIds.has(n.id);
    const rm = (n.config.responseMap as Array<{ contextVariable?: ContextVariable; type?: string }>) ?? [];
    rm.forEach(row => {
      ...
      vars.push({ name: cv.name, type: row.type ?? DataType.STRING, sourceNode: n.label, scope: cv.scope });
    });
  });

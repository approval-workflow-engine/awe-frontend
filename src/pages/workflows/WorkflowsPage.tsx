...
export default function WorkflowsPage() {
  ...
  const handleOpenBuilder = async (workflowId: string) => {
    console.log("Open Builder clicked", workflowId);

    try {
      const res = await getLatestWorkflowVersion(workflowId);

      const version = res.data.data.version;

      console.log("Version fetched", version, res.data.data);

      if (version) {
        navigate(`/workflows/${workflowId}/builder/${version}`);
        return;
      } else {
        const res = await createWorkflowVersion(workflowId);
        console.log("Version created", res.data.version, res.data);
        if (!version) {
          console.error("Version not returned from API", res.data);
          return;
        }

        navigate(`/workflows/${workflowId}/builder/${res.data.version}`);
      }
    } catch (error) {
      console.log("Failed to fetch workflow version", error);
    }
  };

  const fetchWorkflows = useCallback(
    async (pg: number, f: string) => {
      setListLoading(true);
      try {
        ...
        if (res) {
          ...
          const deduped = list.filter(
            (wf, idx, self) => self.findIndex((w) => w.id === wf.id) === idx,
          );
  ...
  const handleLaunchSubmit = async () => {
    ...
    for (const inp of launchInputs) {
      ...
      if (inp.datatype === "number") {
        ...
        try {
          context[key] = JSON.parse(inp.value);
        } catch {
          setLaunchError(`"${key}" must be valid JSON for object type.`);
    ...
    const contextToSend = launchInputs.some((i) => i.name.trim())
      ? context
      : undefined;
    const res = await call(
      () =>
        startInstance({
          workflowId: launchTarget.id,
          context: contextToSend,
          autoAdvance: launchAutoAdvance,
        }),
      {
        showError: false,
        onError: (err) => {
  ...
  const displayedWorkflows = isSearchActive
    ? (allWorkflows || []).filter((wf) => {
        ...
        const updatedDate = wf.updatedAt
          ? new Date(wf.updatedAt as string).toLocaleDateString("en-GB", {
              day: "2-digit",
              month: "short",
              year: "numeric",
        ...
        const createdDate = wf.createdAt
          ? new Date(wf.createdAt as string).toLocaleDateString("en-GB", {
              day: "2-digit",
              month: "short",
              year: "numeric",
              timeZone: "Asia/Kolkata",
  ...
  return (
    <Box>
                ...
                displayedWorkflows.map((wf) => (
                  ...
                  >
                    ...
                    <TableCell>
                      ...
                      >
                        {wf.createdAt
                          ? new Date(wf.createdAt as string).toLocaleDateString(
                              "en-GB",
                              {
                                day: "2-digit",
                                month: "short",
                    ...
                    <TableCell>
                      ...
                      >
                        {wf.updatedAt
                          ? new Date(wf.updatedAt as string).toLocaleDateString(
                              "en-GB",
                              {
                                day: "2-digit",
                                month: "short",
            ...
            (allWorkflows || []).length > 0 && (
              ...
              >
                <Typography sx={{ fontSize: 12, color: "text.disabled" }}>
                  ...
                  workflow{(allWorkflows || []).length !== 1 ? "s" : ""}
                </Typography>
              </Box>
            )
          : total > 0 && (
                ...
                {totalPages > 1 && (
                  <Pagination
                    ...
                    sx={{
                      ...
                      "& .Mui-selected": {
                        backgroundColor: "rgba(79,110,247,0.15) !important",
                        color: "primary.main",
                      },
                    }}
                  />
      ...
      <Dialog
        open={launchOpen}
        onClose={() => {
          if (!launchLoading) setLaunchOpen(false);
        }}
        maxWidth="sm"
        fullWidth
      >
            ...
            <Box display="flex" flexDirection="column" gap={1} mb={1.5}>
              {launchInputs.map((inp, i) => (
                ...
                >
                  ...
                  >
                    {(["string", "number", "boolean", "object"] as const).map(
                      (t) => (
          ...
          >
            <Box>
              ...
              <Typography sx={{ fontSize: 11, color: "text.secondary" }}>
                Automatically execute automated nodes on start
              </Typography>
            </Box>
            <Switch
